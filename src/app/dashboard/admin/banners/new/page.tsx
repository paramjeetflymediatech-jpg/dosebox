      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        <form onSubmit={handleCreateBanner} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 space-y-2">
            <label className="text-sm font-semibold text-slate-700">Banner Image (Upload or URL)</label>
            ...
            <input type="url" placeholder="Image URL (if not uploading)" value={newBanner.image} onChange={e => setNewBanner({...newBanner, image: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-brand-500 transition-colors" />
          </div>
          ...
          <div className="md:col-span-2 mt-4 flex justify-end">
            <button type="submit" disabled={loading || (!imageFile && !newBanner.image)} className="px-8 py-3 bg-brand-600 text-white font-semibold rounded-xl hover:bg-brand-700 transition-colors shadow-sm flex items-center gap-2 disabled:opacity-50">
