...
import { 
  DollarSign, ShoppingBag, Users, AlertTriangle, Download, Activity, FileText, Pill, Wand2
} from 'lucide-react';
...
function MedicoDashboard({ user }: any) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setStats(res.data.data.medicoStats);
      } catch (err) {
        console.error('Failed to load medico stats', err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="flex h-[calc(100vh-64px)] items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 text-sm font-semibold">Loading Medical Portal...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
        <div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-slate-600 tracking-tight">Medical Portal</h1>
          <p className="text-slate-500 mt-2 font-medium text-lg">Welcome back, Dr. {user?.name}</p>
        </div>
        <div className="flex gap-3 mt-4 md:mt-0">
          <Link 
            href="/dashboard/admin/generate-ai-bulk"
            className="px-6 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 text-white rounded-xl shadow-lg shadow-brand-200 hover:shadow-brand-300 hover:-translate-y-0.5 transition-all font-semibold flex items-center gap-2"
          >
            <Wand2 className="w-4 h-4" /> AI Bulk Generation
          </Link>
        </div>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Link href="/dashboard/admin/prescriptions" className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl shadow-lg shadow-slate-200/50 border border-white flex flex-col justify-between group hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-100 transition-all duration-300 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-brand-100/50 rounded-full blur-3xl -mr-10 -mt-10 transition-transform group-hover:scale-110"></div>
          <div className="flex items-center justify-between mb-4 relative z-10">
            <div className="w-14 h-14 bg-gradient-to-br from-brand-100 to-brand-50 rounded-2xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-300">
              <FileText className="w-7 h-7 text-brand-600 drop-shadow-sm" />
            </div>
          </div>
          <div className="relative z-10">
            <p className="text-sm font-semibold text-slate-500 mb-1 tracking-wide uppercase">Pending Prescriptions</p>
            <h3 className="text-3xl font-black text-slate-800 tracking-tight">{stats?.pendingPrescriptions || 0}</h3>
          </div>
        </Link>
        
        <Link href="/dashboard/admin/medicines" className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl shadow-lg shadow-slate-200/50 border border-white flex flex-col justify-between group hover:-translate-y-1 hover:shadow-xl hover:shadow-amber-100 transition-all duration-300 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <div className="w-14 h-14 bg-gradient-to-br from-amber-100 to-amber-50 rounded-2xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-300">
              <Pill className="w-7 h-7 text-amber-600 drop-shadow-sm" />
            </div>
          </div>
          <div className="relative z-10">
            <p className="text-sm font-semibold text-slate-500 mb-1 tracking-wide uppercase">Draft Content</p>
            <h3 className="text-3xl font-black text-slate-800 tracking-tight">{stats?.draftMedicines || 0}</h3>
          </div>
        </Link>
        
        <Link href="/dashboard/admin/medicines" className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl shadow-lg shadow-slate-200/50 border border-white flex flex-col justify-between group hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-100 transition-all duration-300 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-100/50 rounded-full blur-3xl -mr-10 -mt-10 transition-transform group-hover:scale-110"></div>
          <div className="flex items-center justify-between mb-4 relative z-10">
            <div className="w-14 h-14 bg-gradient-to-br from-indigo-100 to-indigo-50 rounded-2xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-300">
              <Activity className="w-7 h-7 text-indigo-600 drop-shadow-sm" />
            </div>
          </div>
          <div className="relative z-10">
            <p className="text-sm font-semibold text-slate-500 mb-1 tracking-wide uppercase">In Review</p>
            <h3 className="text-3xl font-black text-slate-800 tracking-tight">{stats?.reviewMedicines || 0}</h3>
          </div>
        </Link>
        
        <Link href="/dashboard/admin/medicines" className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl shadow-lg shadow-slate-200/50 border border-white flex flex-col justify-between group hover:-translate-y-1 hover:shadow-xl hover:shadow-rose-100 transition-all duration-300 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <p className="text-sm font-semibold text-slate-500 mb-1 tracking-wide uppercase">Low Stock</p>
            <h3 className="text-3xl font-black text-rose-600 tracking-tight">{stats?.lowStockMedicines || 0}</h3>
          </div>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative z-10">
        ...
        <div className="bg-white/80 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-64 h-64 bg-brand-100/30 rounded-full blur-[80px] -mr-20 -mt-20 pointer-events-none transition-opacity group-hover:opacity-100 opacity-50"></div>
          <div className="flex justify-between items-end mb-8 relative z-10">
            <div>
              ...
              <p className="text-slate-500 font-medium mt-1 ml-12">Current state of prescription approvals</p>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  cornerRadius={8}
                >
                  {(stats?.prescriptionChart || []).map((entry: any, index: number) => {
                    const colors = ['#f59e0b', '#10b981', '#f43f5e', '#6366f1'];
        ...
        {/* Catalog Content Status Chart */}
        <div className="bg-white/80 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-100/30 rounded-full blur-[80px] -mr-20 -mt-20 pointer-events-none transition-opacity group-hover:opacity-100 opacity-50"></div>
          <div className="flex justify-between items-end mb-8 relative z-10">
            <div>
              <h3 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3">
               ...
               Medicines Catalog Content Status
              </h3>
              <p className="text-slate-500 font-medium mt-1 ml-12">Distribution of medical content states</p>
            </div>
          </div>
          <div className="h-[350px] w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  cornerRadius={8}
                >
                  {(stats?.catalogChart || []).map((entry: any, index: number) => {
                    const colors = ['#10b981', '#6366f1', '#475569', '#f43f5e'];
                    return <Cell key={`cell-${index}`} fill={colors[index % colors.length]} className="hover:opacity-80 transition-opacity cursor-pointer" />;
                  })}
                </Pie>
                <Tooltip 
...
function LeadershipDashboard({ user }: any) {
  ...
  useEffect(() => {
    const loadAdminData = async () => {
      ...
      try {
        ...
        console.error('Failed to load leadership stats', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleExport = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', 'dosebox_business_report.xlsx');
    ...
    return (
      <div className="flex h-[calc(100vh-64px)] items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 text-sm font-semibold">Loading Business Insights...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium text-lg">Executive Summary for {user?.name}</p>
        </div>
        <div className="flex gap-3 mt-4 md:mt-0">
          <button 
            onClick={handleExport}
            className="px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white rounded-xl shadow-lg shadow-indigo-200 hover:shadow-indigo-300 hover:-translate-y-0.5 transition-all font-semibold flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white/80 backdrop-blur-xl p-8 rounded-3xl shadow-lg shadow-slate-200/50 border border-white flex flex-col justify-between group hover:-translate-y-1 hover:shadow-xl hover:shadow-emerald-100 transition-all duration-300 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <div className="w-16 h-16 bg-gradient-to-br from-emerald-100 to-emerald-50 rounded-2xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-300">
              <DollarSign className="w-8 h-8 text-emerald-600 drop-shadow-sm" />
            </div>
          </div>
          <div className="relative z-10">
            <p className="text-sm font-semibold text-slate-500 mb-1 tracking-wide uppercase">Gross Revenue</p>
            <h3 className="text-4xl font-black text-slate-800 tracking-tight">₹{formatCurrency(kpis?.totalRevenue || 0)}</h3>
          </div>
        </div>

        <div className="bg-white/80 backdrop-blur-xl p-8 rounded-3xl shadow-lg shadow-slate-200/50 border border-white flex flex-col justify-between group hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-100 transition-all duration-300 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <div className="w-16 h-16 bg-gradient-to-br from-blue-100 to-blue-50 rounded-2xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-300">
              <ShoppingBag className="w-8 h-8 text-blue-600 drop-shadow-sm" />
            </div>
          </div>
          <div className="relative z-10">
            <p className="text-sm font-semibold text-slate-500 mb-1 tracking-wide uppercase">Total Orders Processed</p>
            <h3 className="text-4xl font-black text-slate-800 tracking-tight">{kpis?.totalOrders.toLocaleString()}</h3>
          </div>
        </div>

        <div className="bg-white/80 backdrop-blur-xl p-8 rounded-3xl shadow-lg shadow-slate-200/50 border border-white flex flex-col justify-between group hover:-translate-y-1 hover:shadow-xl hover:shadow-purple-100 transition-all duration-300 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <div className="w-16 h-16 bg-gradient-to-br from-purple-100 to-purple-50 rounded-2xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-300">
              <Users className="w-8 h-8 text-purple-600 drop-shadow-sm" />
            </div>
          </div>
          <div className="relative z-10">
            <p className="text-sm font-semibold text-slate-500 mb-1 tracking-wide uppercase">Active Customers</p>
            <h3 className="text-4xl font-black text-slate-800 tracking-tight">{kpis?.totalCustomers.toLocaleString()}</h3>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 relative overflow-hidden group">
          ...
          <div className="flex justify-between items-end mb-8 relative z-10">
            <div>
              ...
              <p className="text-slate-500 font-medium mt-1 ml-12">Year-to-date performance</p>
            </div>
          </div>
          <div className="h-[400px] w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueChart} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                <defs>
                  <linearGradient id="colorRevLead" x1="0" y1="0" x2="0" y2="1">
                ...
                />
                <Area type="monotone" dataKey="revenue" stroke="#10b981" strokeWidth={4} fillOpacity={1} fill="url(#colorRevLead)" activeDot={{ r: 6, strokeWidth: 0, fill: '#10b981' }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 relative overflow-hidden group">
          ...
          <div className="flex justify-between items-end mb-8 relative z-10">
            <div>
              ...
              <p className="text-slate-500 font-medium mt-1 ml-12">Breakdown of operational efficiency</p>
            </div>
          </div>
          <div className="h-[400px] w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
...
function AdminDashboard({ user }: any) {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
        <div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-slate-600 tracking-tight">
