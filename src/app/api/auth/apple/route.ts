...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!appleId) {
      return NextResponse.json({ success: false, message: 'Apple authentication ID missing' }, { status: 400 });
    }

    // Attempt to locate user by appleId or email
    let user = null;
    if (email) {
    ...
    const userEmail = email || `${appleId.substring(0, 10)}@privaterelay.appleid.com`;
    ...
    if (!user) {
      ...
      if (name && user.name !== name && user.name === 'Apple User') {
        user.name = name;
        needsSave = true;
      }
      if (needsSave) {
    ...
    if (body.device_id || body.device_platform) {
      console.log('Mobile Apple login detected!', { device_id: body.device_id, device_platform: body.device_platform });
      try {
        ...
        console.log('Mobile Apple auth tracked successfully!');
      } catch (err) {
