...
export default function AdminAppointmentsScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      setTimeout(() => Alert.alert('Error', 'Failed to load Appointments'), 100);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleOpenModal = (item = null) => {
    ...
    setFormData({
      patientName: item ? (item.patientName !== undefined ? String(item.patientName) : '') : '',
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && currentId) {
        await api.put('/admin/appointments/' + currentId, formData);
      } else {
      ...
      console.log('Failed to save Appointments:', err);
      setTimeout(() => Alert.alert('Error', 'Failed to save Appointments'), 100);
    } finally {
      setSaving(false);
    }
  };
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardBody}>
        <Text style={styles.cardTitle}>{item.patientName || "Unknown Patient" || 'Untitled'}</Text>
        <Text style={styles.cardSubtitle} numberOfLines={2}>
          {item.status || 'No description available'}
        </Text>
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{isEditing ? 'Edit' : 'Add'} Appointments</Text>
            ...
            <FlatList
              ...
              <View style={styles.fieldBox}>
                ...
                <TextInput
                  ...
                  onChangeText={txt => setFormData({...formData, patientName: txt})}
