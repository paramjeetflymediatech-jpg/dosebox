...
export default function AdminSEOScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      console.log('Error loading Route SEO:', err);
      AlertService.show({ type: 'error', title: 'Error', message: 'Failed to load Route SEO' });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleOpenModal = (item = null) => {
    ...
    setFormData({
      route: item ? (item.route !== undefined ? String(item.route) : '') : '',
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && currentId) {
        await api.put('/admin/page-meta/' + currentId, formData);
      } else {
        await api.post('/admin/page-meta', formData);
      }
      setModalVisible(false);
      loadData();
    } catch (err) {
      console.log('Failed to save Route SEO:', err);
      AlertService.show({ type: 'error', title: 'Error', message: 'Failed to save Route SEO' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardBody}>
        <Text style={styles.cardTitle}>{item.route || 'Untitled'}</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Route SEO</Text>
      ...
      <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>No route seo found.</Text>
            </View>
          }
          ListFooterComponent={
            <Pagination 
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{isEditing ? 'Edit' : 'Add'} Route SEO</Text>
            ...
            <ScrollView contentContainerStyle={styles.formScroll}>
              <View style={styles.fieldBox}>
                ...
                <TextInput
                  ...
                  onChangeText={txt => setFormData({...formData, route: txt})}
