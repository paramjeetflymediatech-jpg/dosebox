...
export default function AdminRewardsScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      console.log('Error loading Rewards Config:', err);
      setTimeout(() => Alert.alert('Error', 'Failed to load Rewards Config'), 100);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleOpenModal = (item = null) => {
    ...
    setFormData({
      signupBonus: item ? (item.signupBonus !== undefined ? String(item.signupBonus) : '') : '',
      referralBonus: item ? (item.referralBonus !== undefined ? String(item.referralBonus) : '') : '',
    });
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && currentId) {
        await api.put('/admin/settings/rewards/' + currentId, formData);
      } else {
        await api.post('/admin/settings/rewards', formData);
      }
      setModalVisible(false);
      loadData();
    } catch (err) {
      console.log('Failed to save Rewards Config:', err);
      setTimeout(() => Alert.alert('Error', 'Failed to save Rewards Config'), 100);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = (id) => {
    Alert.alert('Delete', 'Are you sure you want to delete this item?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
            await api.delete('/admin/settings/rewards/' + id);
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardBody}>
        <Text style={styles.cardTitle}>{"Rewards Configuration" || 'Untitled'}</Text>
        <Text style={styles.cardSubtitle} numberOfLines={2}>
          {"Signup: " + item.signupBonus + " | Referral: " + item.referralBonus || 'No description available'}
        </Text>
      </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Rewards Config</Text>
      ...
      <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>No rewards config found.</Text>
            </View>
          }
          ListFooterComponent={
            <Pagination 
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{isEditing ? 'Edit' : 'Add'} Rewards Config</Text>
            ...
            <FlatList
              ...
              <View style={styles.fieldBox}>
                <Text style={styles.fieldLabel}>Signup Bonus</Text>
                <TextInput
              ...
              <View style={styles.fieldBox}>
                <Text style={styles.fieldLabel}>Referral Bonus</Text>
                <TextInput
                  style={[styles.input, null]}
