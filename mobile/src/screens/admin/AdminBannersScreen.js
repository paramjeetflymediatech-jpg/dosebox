...
export default function AdminBannersScreen({ navigation }) {
  ...
  const handleOpenModal = (item = null) => {
    ...
    setFormData({
      ...
      image: item ? (item.image !== undefined ? String(item.image) : '') : '',
      link: item ? (item.link !== undefined ? String(item.link) : '') : '',
    });
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && currentId) {
        await api.put('/admin/banners/' + currentId, formData);
      } else {
    ...
    <View style={styles.card}>
      <View style={styles.cardBody}>
        ...
        <Text style={styles.cardSubtitle} numberOfLines={2}>
          {item.link || 'No description available'}
        </Text>
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{isEditing ? 'Edit' : 'Add'} Banners</Text>
