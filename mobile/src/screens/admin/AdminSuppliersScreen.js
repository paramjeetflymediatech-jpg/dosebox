...
export default function AdminSuppliersScreen({ navigation }) {
  ...
  const handleOpenModal = (item = null) => {
    ...
    setFormData({
      ...
      phone: item ? (item.phone !== undefined ? String(item.phone) : '') : '',
    });
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && currentId) {
        await api.put('/admin/suppliers/' + currentId, formData);
      } else {
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{isEditing ? 'Edit' : 'Add'} Suppliers</Text>
