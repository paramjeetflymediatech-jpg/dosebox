...
export default function LeadershipDoctorsScreen({ navigation }) {
  ...
  const handleOpenModal = (item = null) => {
    ...
    setFormData({
      ...
      specialization: item ? (item.specialization !== undefined ? String(item.specialization) : '') : '',
    });
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && currentId) {
        await api.put('/admin/doctors/' + currentId, formData);
      } else {
      ...
      console.log('Failed to save Doctors:', err);
      AlertService.show({ type: 'error', title: 'Error', message: 'Failed to save Doctors' });
    } finally {
      setSaving(false);
    }
  };
    ...
    <View style={styles.card}>
      <View style={styles.cardBody}>
        ...
        <Text style={styles.cardSubtitle} numberOfLines={2}>
          {item.specialization || item.email || 'No description available'}
        </Text>
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{isEditing ? 'Edit' : 'Add'} Doctors</Text>
