...
export default function LeadershipSupportScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      AlertService.show({ type: 'error', title: 'Error', message: 'Failed to load FAQs' });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleOpenModal = (item = null) => {
    ...
    setFormData({
      question: item ? (item.question !== undefined ? String(item.question) : '') : '',
      answer: item ? (item.answer !== undefined ? String(item.answer) : '') : '',
      displayOrder: item ? (item.displayOrder !== undefined ? String(item.displayOrder) : '') : '',
    });
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && currentId) {
        await api.put('/admin/faqs/' + currentId, formData);
      } else {
      ...
      console.log('Failed to save FAQs:', err);
      AlertService.show({ type: 'error', title: 'Error', message: 'Failed to save FAQs' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardBody}>
        <Text style={styles.cardTitle}>{item.question || 'Untitled'}</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Support & Bot Simulator</Text>
      ...
      {loading ? (
        ...
        <>
      <View style={{ backgroundColor: '#F0FDF4', padding: spacing.md, marginHorizontal: spacing.md, marginBottom: rv(16), borderRadius: radius.md, borderWidth: 1, borderColor: '#BBF7D0', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: rm(14), fontWeight: '700', color: '#166534' }}>Bot Simulator</Text>
          <Text style={{ fontSize: rm(12), color: '#15803D', marginTop: rv(2) }}>Test automated polite responses.</Text>
        </View>
        <TouchableOpacity style={{ backgroundColor: '#22C55E', paddingHorizontal: rs(12), paddingVertical: rv(8), borderRadius: radius.sm }}>
          <Text style={{ color: '#fff', fontWeight: '600', fontSize: rm(12) }}>Test Bot 🤖</Text>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{isEditing ? 'Edit' : 'Add'} FAQs</Text>
            ...
            <FlatList
              ...
              <View style={styles.fieldBox}>
                ...
                <TextInput
                  ...
                  onChangeText={txt => setFormData({...formData, question: txt})}
              ...
              <View style={styles.fieldBox}>
                ...
                <TextInput
                  ...
                  onChangeText={txt => setFormData({...formData, answer: txt})}
              ...
              <View style={styles.fieldBox}>
                ...
                <TextInput
                  ...
                  onChangeText={txt => setFormData({...formData, displayOrder: txt})}
