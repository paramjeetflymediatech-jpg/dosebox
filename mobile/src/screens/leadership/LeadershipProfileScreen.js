...
export default function LeadershipProfileScreen({ navigation }) {
  ...
  const fetchProfile = async () => {
    try {
      ...
      console.log('Failed to fetch leadership profile:', err);
    }
  };
  ...
  const initials = user?.name ? user.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) : 'L';
      ...
      <Modal visible={showLogoutModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalMessage}>Are you sure you want to securely end your executive session?</Text>
...
const styles = StyleSheet.create({
  ...
  editBtnText: { color: '#1F5C52', fontWeight: '800', fontSize: rm(12) },
